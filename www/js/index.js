...
const titikMerah = document.querySelector('.titik_merah');
...
async function getLocation() {
  const suc = async function (p) {
    let locs = await addLocation(p.coords.latitude, p.coords.longitude);
...
async function addLocation(lat, long) {
  let check = await checkLocation(lat, long);
  if (check) {
    return false;
  }
...
if (btnLocation) {
  btnLocation.addEventListener('click', async () => {
    const getLoc = await getLocation();
    if (getLoc) {
      setTimeout(() => {
